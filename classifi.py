...
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sampling/log'))
...
from sampling.models.Sampling import InfoSampling
...
def parse_args():
    ...
    parser.add_argument('--num_point_skel', type=int, default=1024, help='Point Number Skeleton')
    ...
    parser.add_argument('--spl_batch_size', type=int, default=2, help='batch size in training')
    parser.add_argument('--use_pretrained_skeleton', action='store_false', default=True, help='use pretrain model')
    parser.add_argument('--pretrain_skeleton', type=str, default='', help='')
    parser.add_argument('--num_skeleton_point', type=int, default=1024, help='Skeleton Point Number')
    parser.add_argument('--num_sampling', type=int, default=16, metavar='N', help='Num of sampling points')
...
def train(args):
    ...
    skel_data_path = '../data/modelnet/ModelNetSkel'
    train_dataset = ModelNetDataLoader(root=data_path, root_skel=skel_data_path, args=args, split='train')
    test_dataset = ModelNetDataLoader(root=data_path, root_skel=skel_data_path, args=args, split='test')
    ...
    #     try:
    #         device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    #         checkpoint = torch.load('./log/skeleton_2024-06-27-17-46-39/checkpoints_skeletons/skeleton_0080epoch.pth', map_location=device)
    ...
    try:
        checkpoint = torch.load('sampling/log/16_MS3D/checkpoints_sampling/sampling_best.pth', map_location=device)
        model_spl.load_state_dict(checkpoint['model_state_dict'], )
        print('Load pretrained sampling model')
    except:
        print('Error: No existing sampling model')
        exit()
    ...
    for epoch in range(start_spl_epoch, spl_epoch):
        ...
        for batch_id, (points, ske_xyz, target, pnames, dly_idx) in tqdm(enumerate(train_loader_spl, 0), total=len(train_loader_spl), smoothing=0.9):
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, ske_xyz, target, pnames, dly_idx) in tqdm(enumerate(test_loader_spl), total=len(test_loader_spl)):
                points, target = points.float().to(device), target.long().to(device)
                dly_idx = dly_idx.long().to(device)
